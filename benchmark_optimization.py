...
def main():   
    ...
    with Parallel(n_jobs=len(dataset_bodies_reduced)) as parallel:
        parallel(delayed(model_builder_v1.optimize_neuron_positions)(dataset_bodies_reduced[i], dataset_synapses, 'output/optimized_cell_positions_benchmark_'+str(i)+'.pickle')\
